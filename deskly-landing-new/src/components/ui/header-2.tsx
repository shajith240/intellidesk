		...
		<div className="fixed top-0 left-0 right-0 z-50 flex justify-center">
			<header
				className={cn(
					...
					scrolled && !open
						? "mt-3 h-12 w-[min(56rem,calc(100%-2rem))] rounded-2xl border border-white/[0.06] bg-background/60 px-5 shadow-lg shadow-black/20 backdrop-blur-xl"
						: "mt-0 h-14 w-full border-b border-white/[0.06] bg-transparent px-6",
					open &&
						"mt-0 h-14 w-full bg-background/90 border-b border-white/[0.06] px-6",
				)}
			>
			...
			<div
				className={cn(
					"bg-background/95 fixed top-14 right-0 bottom-0 left-0 z-40 flex flex-col overflow-hidden border-y backdrop-blur-xl md:hidden",
					open ? "block" : "hidden",
				)}
			>
				<div
